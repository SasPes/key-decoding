...
// --- GLOBAL DATABASE (US, EU, AND MIRRORED/DOUBLE SIDED) ---
var keys = {
    // --- EU Standards (Scharfe V-Flanken, Standard-Abstand) ---
    BKS: {
        displayName: "BKS PZ88",
        outlines: ["5 pins", "6 pins"],
        pinSpacing: 28,
        maxKeyCut: 10,
        flatSpotWidth: 2,       // Spitze Zacken
        cutDepthOffset: 4,      // Skaliert visuell sauber
        zeroCutOffset: 0,
        isEuropean: true,
    ...
    // --- Doppelseitig Gespiegelte Schlüssel (Automotive/Fahrrad/Wendeschlüssel) ---
    DoubleMirror: {
        displayName: "EU Double-Sided",
        ...
        zeroCutOffset: 1,
        isEuropean: true,
        isDoubleSided: true,     // Aktiviert die visuelle Spiegelung nach unten
        pinsStartAtZero: false
    },
    // --- USA Standards (Flachere Winkel, breitere Täler) ---
    Schlage: {
...
function Key(type, outline, show) {
    ...
    this.save = function () {
        var data = { type: this.type, outline: this.outline, pins: this.pins };
        var fileName = "/keys/key_" + this.type + "_" + this.pins.join('') + "_" + Date.now() + ".json";
        if (storage.write(fileName, JSON.stringify(data))) {
...
// Dynamische Flankenberechnung (Scharf vs. Rund)
function generateDipShapes(pinSpacing, maxKeyCut, flatSpotWidth, cutDepthOffset, zeroCutOffset, isEuropean) {
    var dipShapes = {};
    for (var cut = 0; cut < maxKeyCut; cut++) {
        var shape = [];
        var centerIndex = Math.floor(pinSpacing / 2);
        var cutDepth = cut * cutDepthOffset;
        var flatHalfWidth = isEuropean ? Math.floor(flatSpotWidth / 4) : Math.floor(flatSpotWidth / 2);

        for (var i = 0; i < pinSpacing; i++) {
            var distanceFromCenter = Math.abs(i - centerIndex);
            if (cut === 0) {
                shape[i] = (distanceFromCenter <= flatHalfWidth) ? zeroCutOffset : 0;
            } else {
                if (distanceFromCenter <= flatHalfWidth) {
                    ...
                    // Mathematische V-Form für EU, Sinus/Rundung für USA
                    var depth = isEuropean 
                        ? (cutDepth - Math.floor(slopeDistance * cutDepth / remainingDistance))
                        : (cutDepth - Math.floor(slopeDistance * (cutDepth - 1) / remainingDistance));
                    shape[i] = Math.max(0, depth);
                }
            }
        }
        dipShapes[cut] = shape;
...
function drawKeyShape(x, y, width, height, color, pinCount, pins, keyType) {
    ...
    var isEuropean = keyConfig.isEuropean === true;
    var isDoubleSided = keyConfig.isDoubleSided === true;

    var dipShapes = generateDipShapes(pinSpacing, maxKeyCut, flatSpotWidth, cutDepthOffset, zeroCutOffset, isEuropean);

    var baseLineY = isDoubleSided ? (y + Math.floor(height / 2)) : y;
    ...
    for (var px = Math.round(x); px <= Math.round(x + width + pinSpacing / 2); px++) {
        var pyTop = baseLineY;
        var pyBottom = baseLineY + 20; // Blinddicke bei zweiseitigen Schlüsseln
        ...
        for (var i = 0; i < pinCount; i++) {
            ...
            if (dipShape) {
                ...
                if (px >= dipStart && px < dipEnd) {
                    ...
                    if (isDoubleSided) {
                        pyTop = baseLineY - dipShape[dipIdx];    // Gespiegelt nach oben ausbrechen
                        pyBottom = baseLineY + 20 + dipShape[dipIdx]; // Gespiegelt nach unten ausbrechen
                    } else {
                        pyTop = baseLineY + dipShape[dipIdx];   // Standard-Rücken nach unten vertiefen
                    }
                    break;
                }
            }
        ...
        if (isDoubleSided) {
            display.drawPixel(px, pyTop, color);
            display.drawPixel(px, pyBottom, color);
        } else {
            display.drawPixel(px, pyTop, color);
        }
    }

    // Abschlusskanten zeichnen
    var edgeX = x + width + pinSpacing / 2 + edgeOffsetX;
    if (isDoubleSided) {
        // Zweiseitige Spitze abrunden/schließen
        display.drawLine(edgeX, baseLineY - 5, edgeX + 15, baseLineY + 10, color);
        display.drawLine(edgeX, baseLineY + 25, edgeX + 15, baseLineY + 10, color);
        display.drawLine(x, baseLineY - 5, x, baseLineY + 25, color);
    } else {
        // Einseitige Standard-Schulterschließung
        var edgeY = y + height + edgeOffsetY;
        display.drawLine(edgeX, edgeY, edgeX + 30, edgeY - 30, color);
...
function drawPinsWithUnderline(pins, selectedPinIndex, showMode, pinSpacing, keyType) {
    ...
    drawKeyShape(keyX, keyY, totalWidth, 50, priColor, pins.length, pins, keyType);
}
...
function drawDiskKeyShape(x, y, width, height, color, diskCount, disks, keyType) {
    ...
    for (var i = 1; i < diskCount; i++) {
        var cutDepth = diskCutDepths[disks[i] || 0] || 0;
    ...
    display.drawLine(entryX, bladeBottom, entryX + 20, bladeBottom, color);
    display.drawLine(entryX, bladeY, currX + pinSpacing, bladeY, color);
}
...
function drawDisksWithUnderline(disks, selectedDiskIndex, showMode, pinSpacing, keyType) {
    ...
    for (var i = 0; i < disks.length; i++) {
        var diskNumberX = startX + 12 + i * pinSpacing;
    ...
    drawDiskKeyShape(startX - pinSpacing / 2, startY + pinSpacing, totalWidth, 66, priColor, disks.length, disks, keyType);
}
...
function chooseAndCreateKey() {
    ...
    for (var i = 0; i < brandNames.length; i++) {
        ...
        keyTypeChoices[keys[brand].displayName || brand] = brand;
    }
    ...
    if (type !== "Exit") {
        ...
        if (type === "Load") {
            ...
            key.load(storage.read(filePath));
        }
        key.draw();
    }
}
...
while (true) {
    ...
    if (keyboard.getSelPress()) {
        if (key.show === "random") chooseAndCreateKey();
        else { selectedPinIndex++; key.draw(); }
    }

    if (keyboard.getNextPress()) {
        if (key.show === "random") {
            key.updatePins();
        } else if (key.show === "decode" && selectedPinIndex < key.pins.length) {
    ...
    if (keyboard.getPrevPress()) {
        if (key.show === "decode" && selectedPinIndex < key.pins.length) {
            key.pins[selectedPinIndex] = Math.max(0, key.pins[selectedPinIndex] - 1);
            key.draw();
        }
    }

    if (keyboard.getEscPress()) chooseAndCreateKey();
    delay(10);
}
